...
/** Insert an ADMIN user directly and return their Bearer token. */
...
describe('POST /api/vehicles', () => {
  ...
  // ── ADMIN authorization (RED — requireAdmin not yet applied to POST) ────────

  // A. USER must receive 403 when attempting to add a vehicle
  ...
  // B. ADMIN must succeed when adding a vehicle
...
// ── PUT /api/vehicles/:id — ADMIN authorization (RED) ────────────────────────

describe('PUT /api/vehicles/:id — admin authorization', () => {

  // C. USER must receive 403 when attempting to update a vehicle
  it('should return 403 when a USER (non-admin) tries to update a vehicle', async () => {
    const adminToken = await getAdminToken();

    // Seed a vehicle via direct DB insert so we are not relying on POST being
    // available to USER (POST is also being guarded in this same cycle).
  ...
  // D. ADMIN must succeed when updating a vehicle
